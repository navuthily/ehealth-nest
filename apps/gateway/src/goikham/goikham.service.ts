...
export class GoiKhamService {
  ...
  async exec_GD2_GoiKham_SelectAll() {
   ...
   ,dmck.Ten as TenChuyenKhoa
	 ,ThanhVien.dbo.module_goikham.module_id
	 ,ThanhVien.dbo.module.module_name
FROM
	[GoiKham] gk
  ...
  LEFT JOIN GD2_Dm_ChuyenKhoa dmck ON gk.ID_ChuyenKhoa = dmck.ID_ChuyenKhoa
	LEFT JOIN ThanhVien.dbo.module_goikham ON gk.ID_GoiKham = ThanhVien.dbo.module_goikham.goikham_id
	LEFT JOIN ThanhVien.dbo.module ON ThanhVien.dbo.module_goikham.module_id = ThanhVien.dbo.module.id
