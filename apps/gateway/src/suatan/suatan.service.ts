...
export class SuatAnService {
  constructor(
    ...
    @InjectRepository(SuatAn, 'SV_FAMILY_')
    private suatanRepo: Repository<SuatAn>,
    @InjectRepository(ChiTietSuatAn, 'SV_FAMILY_')
    private chitietsuatanRepo: Repository<ChiTietSuatAn>,
  ...
  async getSuatAnByIdPhieu(id_phieu: number) {
    ...
    for (let i = 0; i < data.length; i++) {
      const nguoitao = await (
        await this.userRepo.findOneOrFail(data[i].Id_NguoiTao)
      ).nickname;
  ...
  async getSuatAnByLkBuoiNgayLoai(id_luotkham, buoi, ngay, loai) {
    ...
    const suatan = await this.SV_FAMILYconnection.getRepository(SuatAn)
      ...
      .where('Pos$ph66_EH.Id_LuotKham = :Id_LuotKham', {
  ...
  async getSuatAnByDay(ngay, id_luotkham: number) {
    const data = await (
      await this.getSuatAn()
    )
      .where('Pos$ph66_EH.ngay_ct = :ngay_ct', { ngay_ct: ngay })
      .andWhere('Pos$ph66_EH.Id_LuotKham = :Id_LuotKham', {
        Id_LuotKham: id_luotkham,
      })
      .getMany();
  ...
  async themsuatan(obj: ThemSuatAnDTO) {
    ...
    const ckeckThongtinluotkham = await this.checkThongtinluotkham(
      obj.Id_LuotKham,
    );
  ...
  async updateSuatan(id_phieu: number, obj) {
    ...
    if (suatAn) {
      ...
      if (obj.chitietsuatan.length === 0) {
        //xoa luon suat an
        const stored = `delete from  FAMILY_WRK.dbo.Pos$ph66_EH
        ...
        //update bảng ph
        const suatAnHienTai = await this.suatanRepo.findOneOrFail({
          Id_Phieu: id_phieu,
        });
  ...
  async functionThemSuatAn(id_phieu, obj: ThemSuatAnDTO) {
    for (let i = 0; i < obj.chitietsuatan.length; i++) {
      ...
      const sqlInsert = `INSERT INTO FAMILY_WRK.dbo.Pos$ct66_EH ("ID_phieu", "Ma_vt", "So_luong", "Gia", "Dvt") 
               VALUES (@0, @1, @2, @3, @4)`;
      const data1 = await this.SV_FAMILYconnection.query(`${sqlInsert}`, [
        id_phieu,
        obj.chitietsuatan[i].Ma_vt,
        obj.chitietsuatan[i].So_luong,
        data[0]['Gia2'],
        data[0]['Dvt'],
      ]);
    }
  ...
  async checkThongtinluotkham(id_luotkham) {
    const stored = `select * from  ThongTinLuotKham
  ...
  //chỉ bảng ct
  async delete(id) {
    const stored = `delete from  FAMILY_WRK.dbo.Pos$ct66_EH
               where ID_phieu = ${id}
          `;
    await this.connection.query(`${stored}`, []);
  }
